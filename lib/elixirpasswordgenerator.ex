defmodule Elixirpasswordgenerator do
  @moduledoc """
  Documentation for `Elixirpasswordgenerator`: Generates a random password depending on parameters, Module main function is 'generate(options)'. This function takes the options map.
  Options examples:
    options = %{
      "length" => "5",
      "numbers" => "false",
  ...
  there are 4 key value pairs in the map; 'length', 'numbers', 'uppercase', and 'symbols'.
  """
  ...
  ## Examples
    ...
    iex> ElixirPasswordGenerator.generate(options)
    "abcdf"
  ...
  defp validate_length_is_integer(false, _options) do
    {:error, "Only integers are allowed for length."}
  end
